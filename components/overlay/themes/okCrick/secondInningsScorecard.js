...
const SecondInningsScorecard = ({ match }) => {
  if (!match) return <div style={{ color: 'white', textAlign: 'center' }}>Loading Match Data...</div>;

  // 2nd Innings ke saved data ya live data ko check karne ke liye logic
    const isSecondInningsSaved = match.secondInningsStatsBackup;
  const isSecondInningsLive = Number(match.innings) === 2; // 👈 Naya check add kiya

  // Agar 2nd innings saved hai ya fir live chal rahi hai tabhi data aayega, warna 1st innings ka data filter ho jayega
  const battingStats = isSecondInningsSaved 
    ? match.secondInningsStatsBackup.batting 
    : (isSecondInningsLive ? (match.battingStats || {}) : {});

  const totalRuns = isSecondInningsSaved 
    ? match.secondInningsStatsBackup.totalRuns 
    : (isSecondInningsLive ? (match.totalRuns || 0) : 0);

  const totalWickets = isSecondInningsSaved 
    ? match.secondInningsStatsBackup.totalWickets 
    : (isSecondInningsLive ? (match.totalWickets || 0) : 0);


    const completedOvers = (match.currentOver || 0) + (match.ballsInOver === 6 ? 1 : 0);

  const oversBowled = isSecondInningsSaved
    ? match.secondInningsStatsBackup.overs
    : (
        isSecondInningsLive // 👈 Check lagaya ki kya 2nd innings live hai
          ? (match.ballsInOver === 6 ? String(completedOvers) : `${match.currentOver || 0}.${match.ballsInOver || 0}`)
          : "0.0"
      );
  
  const extrasData = isSecondInningsSaved 
    ? match.secondInningsStatsBackup.extras 
    : (isSecondInningsLive ? (match.extras?.innings2 || {}) : {}); // 👈 1st innings ke extras block kiye
  ...
  useEffect(() => {
    ...
    const rawBattingPlayers = isSecondInningsSaved 
      ? (match.secondInningsStatsBackup.battingPlayers || []) 
    ...
    }, [match?.battingStats, match?.battingPlayers, isSecondInningsSaved, isSecondInningsLive]);
      ...
      <div className={styles.scorecardContainer}>
        ...
        <div className={styles.matchStatusBar}>
          <div className={styles.matchStatusText}>
            {match.matchNumberSummary || "MATCH 2ND INNINGS"}
          </div>
        </div>
...
export default SecondInningsScorecard;
